  ...
  useEffect(() => {
    ...
    if (parsedData[0].length !== 4) {
      ...
      localFormattingErrors.push({
        row: rowCount,
        error: `Your upload template needs to have 4 columns. Please down the template from the preview page and try again.`,
      });
    ...
    for (var rowCount = 0; rowCount < parsedData.length; rowCount++) {
      if (rowCount !== 0) {
        ...
        if (
          ...
          parsedData[rowCount][2].length > 0
        ) {
          for (
            var colCount = 0;
            ...
            if (colCount === 0) {
              if (parsedData[rowCount][colCount].includes(" ")) {
                hasFormattingErrors = true;
                localFormattingErrors.push({
                  row: rowCount,
                  id: Math.random(),
                  error: `Row ${
                    rowCount + 1
                  } -  The delivery associate ID cannot have a space in it.: ${
              ...
              if (!parsedData[rowCount][colCount].includes(" ")) {
                ...
                localFormattingErrors.push({
                  ...
                  error: `Row ${
                    rowCount + 1
                  } -  The delivery associate must have a full name with a space in it: ${
                    parsedData[rowCount][colCount]
                  }`,
                });
              } else {
            ...
            // Ifts the user phone number then normalize it
            else if (colCount === 3) {
              row[userDataTableColumns[colCount].dataIndex] = normalizePhone(
                parsedData[rowCount][colCount]
              );
            } else {
              row[userDataTableColumns[colCount].dataIndex] =
