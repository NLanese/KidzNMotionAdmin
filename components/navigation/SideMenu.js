  ...
  return (
    <SideMenuWrapper>
      <StyledMenu mode="inline" openKeys={openKeys} selectedKeys={selectedKeys}>
        ...
        <Menu.Item
          onClick={() => pushLink("/shift-management")}
        ...
        <Menu.SubMenu
          key={"/performance"}
          onTitleClick={() => appendMenuKey("/performance")}
        ...
        >
          {user.role === "OWNER" && (
            <Menu.Item
              ...
              onClick={() => pushLink("/performance/overview")}
            >
          ...
          <Menu.Item
            key={"/performance/upload-score-cards"}
            onClick={() => pushLink("/performance/upload-score-cards")}
          >
            Upload Scorecards
          </Menu.Item>
        </Menu.SubMenu>
        <Menu.SubMenu
          key={"/drivers"}
          onTitleClick={() => appendMenuKey("/drivers")}
          title="Driver Management"
          icon={<CodeSandboxOutlined style={{ fontSize: "20px" }} />}
        >
          <Menu.Item
            key={"/drivers/list"}
            onClick={() => pushLink("/drivers/list")}
          >
            View Drivers
          </Menu.Item>
          <Menu.Item
            key={"/drivers/accidents"}
            onClick={() => pushLink("/drivers/accidents")}
          >
            View Accidents
          </Menu.Item>
          {user.role === "OWNER" && (
            <Menu.Item
              key={"/drivers/upload"}
              onClick={() => pushLink("/drivers/upload")}
            >
              Bulk Upload Drivers
            </Menu.Item>
          )}
        ...
        <Menu.SubMenu
          ...
          onTitleClick={() => appendMenuKey("/assets")}
          icon={<BarcodeOutlined style={{ fontSize: "20px" }} />}
        >
          <Menu.Item
            onClick={() => pushLink("/assets/list")}
            key={"/assets/list"}
          >
            View Assets
          </Menu.Item>
          <Menu.Item
            key={"/assets/upload"}
            onClick={() => pushLink("/assets/upload")}
          >
            Bulk Upload Assets
          </Menu.Item>
        </Menu.SubMenu>
        ...
        >
          {user.role === "OWNER" && (
            <>
              <Menu.Item
                key={"/account/company-preferences"}
                onClick={() => pushLink("/account/company-preferences")}
              >
                Company Preferences
              </Menu.Item>
              <Menu.Item
          ...
          <Menu.Item disabled key={"/account/profile-settings"}>
            Profile Settings
          </Menu.Item>
        </Menu.SubMenu>
