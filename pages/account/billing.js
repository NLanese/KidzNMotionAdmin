...
import { getCheckoutURL, getBillingInformation } from "@helpers/billing";

import { useMutation } from "@apollo/client";
import { CHANGE_OWNER_PAYMENT_STATUS } from "@graphql/operations";
...
function Billing() {
  ...
  // Mutations
  const [ownerChangePaymentStatus, {}] = useMutation(
    CHANGE_OWNER_PAYMENT_STATUS
  );

  const fetchAndSetBillingInformation = async (stripeCustomerID) => {
    const billingInformation = await getBillingInformation(stripeCustomerID);
  ...
  useEffect(() => {
    ...
    if (user.dsp.stripeCustomerId) {
      setOnFreeTrial(false);
      fetchAndSetBillingInformation(user.dsp.stripeCustomerId);
      return;
    }
  }, []);

  const updatePaymentStatus = async (sessionID) => {
    await ownerChangePaymentStatus({
      variables: {
        token: localStorage.getItem("token"),
        subscriptionToken: "Activated",
